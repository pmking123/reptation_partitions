...
Usage
-----
    python3 crossover.py [--Ne 10,20,30] [--s 1] [--Lmax 5000]
"""
...
def analyse_ne(Ne, s, L_max, p_unrestricted):
    ...
    Two crossover scales are relevant:
      L*_GC    = (6/pi^2)*Ne^2  -- grand canonical estimate, where E[k]_GC = Ne.
      L*_canon = solution of (sqrt(6)/2pi)*sqrt(L)*log(L) = Ne  -- canonical estimate.
    L*_canon is the correct physical crossover (Section 3 of the paper);
    L*_GC overestimates it by a factor that grows from ~1.3 at Ne=10 to ~4.4 at Ne=85.

    We show two asymptotic predictions:
      Exponential: log P_L ~ alpha_0 * sqrt(L)        (L << L*_canon)
      Polynomial:  log P_L ~ (Ne-1) * log(L/s)        (L >> L*_canon)

    The polynomial form follows from p(L, <=Ne, >=s) ~ (L/s)^{Ne-1}/(Ne!(Ne-1)!).
    """
    ...
    print(
        f"  L* = (6/pi^2)*Ne^2 = {L_star_theory:.1f}  (grand canonical crossover scale)"
    )
    print(f"{'=' * 65}")
    ...
    for L in L_vals:
        ...
        print(
            f"  {L:>6}  {L / L_star_theory:>6.2f}  {log_P:>10.4f}  "
...
def main():
    ...
    parser.add_argument(
        "--Ne",
        default="10,20,30,50,75,85,100,200",
        help="Comma-separated Ne values (default: 10,20,30,50,75,85,100,200)",
    )
    parser.add_argument(
        "--s", type=int, default=1, help="Minimum segment length s (default: 1)"
    )
    parser.add_argument(
        "--Lmax", type=int, default=5000, help="Maximum L to compute (default: 5000)"
    )
    parser.add_argument("--prefix", default="", help="Prefix for output CSV files")
    ...
    print()
    print(f"  Ne   |  L*_GC (theory)  |  L*_canon (correct)")
    print(f"  -----|-----------------|--------------------")
    for Ne in Ne_vals:
        Ls_th = (6.0 / PI**2) * Ne**2
        # Correct canonical crossover: solve (sqrt(6)/2pi)*sqrt(L)*log(L) = Ne
        coeff = math.sqrt(6) / (2 * PI)
        try:
            # Bisection over [2, 1e9]
            lo, hi = 2.0, 1e9
            for _ in range(100):
                mid = (lo + hi) / 2
                if coeff * math.sqrt(mid) * math.log(mid) < Ne:
                    lo = mid
                else:
                    hi = mid
            Ls_cn = (lo + hi) / 2
            ...
            Ls_cn = float("nan")
        print(f"  {Ne:>3}  |  {Ls_th:>15.1f}  |  {Ls_cn:>18.1f}")
    ...
    # Full data
    full_csv = os.path.join(out_dir, f"{args.prefix}_m4_crossover_data.csv")
    if all_rows:
    ...
    # Summary
    summary_csv = os.path.join(out_dir, f"{args.prefix}_m4_crossover_summary.csv")
    ...
    scale_csv = os.path.join(out_dir, f"{args.prefix}_m4_scaled_entropy.csv")
    ...
    print("  has two relevant scales:")
    print(f"    L*_GC    = (6/pi^2) * Ne^2 ~ {6 / PI**2:.3f} * Ne^2  (grand canonical)")
    print(
        "    L*_canon = solution of (sqrt(6)/2pi)*sqrt(L)*log(L) = Ne  (canonical, correct)"
    )
    print()
    print("  L*_canon is the physically correct crossover onset; L*_GC overestimates")
    print("  it by a factor of ~1.3 at Ne=10, growing to ~4.4 at Ne=85.")
    print()
    print("  For L << L*_canon: log P_L^(Ne) ~ alpha_0 * sqrt(L)")
    print("                     (unrestricted regime; constraint rarely active)")
    print()
    print("  For L >> L*_canon: log P_L^(Ne) ~ (Ne-1) * log(L/s)")
    print(
        "                     (confined regime; all configs have exactly Ne segments)"
    )
    print("                     [p(L,<=Ne,>=s) ~ (L/s)^{Ne-1} / (Ne! * (Ne-1)!)]")
