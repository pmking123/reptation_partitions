...
def run_verification(L_max=20, n_random_trials=10, random_seed=42):
    ...
    for label, failures, description in [
        (
            "Test 1",
            failures_t1,
            "R_2 o R_1 (deterministic) always reaches a gap-2 composition",
        ),
        (
            "Test 2",
            failures_t2,
            "R_1 o R_2 (deterministic) always reaches a gap-2 composition",
        ),
        (
            "Test 3",
            failures_t3,
    ...
    print(
        f"  Cases where R_2 o R_1 and R_1 o R_2 reach different attractors: "
        f"{different_attractor_count:,} / {total:,}"
    )
    print(f"  Examples:")
    for comp, fp21, fp12 in different_attractor_examples:
        print(f"    input={comp}  R_2 o R_1->{fp21}  R_1 o R_2->{fp12}")
...
def print_worked_examples():
    ...
    for comp in examples:
        ...
        print(f"  Input:        {comp}")
        print(
            f"  R_2 o R_1 ->     {fp_21}  gap-2: {'YES' if satisfies_gap2(fp_21) else 'NO'}"
        )
        print(
            f"  R_1 o R_2 ->     {fp_12}  gap-2: {'YES' if satisfies_gap2(fp_12) else 'NO'}"
...
if __name__ == "__main__":
    ...
    results = run_verification(L_max=20, n_random_trials=1000, random_seed=42)
    results = run_verification(L_max=25, n_random_trials=10, random_seed=42)
    results = run_verification(L_max=20, n_random_trials=500, random_seed=42)
    results = run_verification(L_max=20, n_random_trials=100, random_seed=1)
    results = run_verification(L_max=20, n_random_trials=100, random_seed=172)
    results = run_verification(L_max=20, n_random_trials=100, random_seed=96)
