...
NOTE: (pi/sqrt(6)) * sqrt(L) is the GRAND CANONICAL mean of k,
        ...
        and Fristedt 1993):
            E[k]_canon ~ (sqrt(6)/(2*pi)) * sqrt(L) * log(L)
            k*_canon   ~ 0.95 * (sqrt(6)/(2*pi)) * sqrt(L) * log(L)
                       ~ 0.370 * sqrt(L) * log(L)
...
def Ek_gc(alpha, j_max=10000):
    ...
    """
    return sum(
        1.0 / (math.exp(j * alpha) - 1) for j in range(1, j_max + 1) if j * alpha < 100
    )
...
def run_analysis(L_max=10000):
    ...
    c = math.sqrt(6) / (2 * PI)
    print(f"  E[k]_canon ~ (sqrt(6)/(2*pi)) * sqrt(L) * log(L)")
    print(f"             = {c:.4f} * sqrt(L) * log(L)")
    print(f"  k*_canon   ~ 0.95 * (sqrt(6)/(2*pi)) * sqrt(L) * log(L)")
    ...
    print()
    print(
        f"  {'L':>6}  {'k*_can':>8}  {'k*/sqL':>8}  {'Ek_can':>8}  "
    ...
    for L in L_vals:
        ...
        results.append({**stats, "alpha": alpha, "Ek_gc": Ek_g, "be_form": be_form})
...
def fit_kstar_scaling(results):
    ...
    for label, basis in [("sqrt(L)*log(sqrt(L))", basis1), ("sqrt(L)*log(L)", basis2)]:
        ...
        print(
            f"    Implied c (= a / (sqrt(6)/(2*pi))): "
            f"{a / (math.sqrt(6) / (2 * math.pi)):.6f}"
        )

    # Convergence of ratio to check which limit is approached
    print()
    print(
        "  Ratio k* / (sqrt(L)*log(L)) for large L "
        "(should converge to sqrt(6)/(2*pi) ~ 0.3899):"
    )
    c_ref = math.sqrt(6) / (2 * math.pi)
    for L, k in zip(L_vals, k_vals):
        r1 = k / (math.sqrt(L) * math.log(math.sqrt(L)))
        r2 = k / (math.sqrt(L) * math.log(L))
        print(
            f"    L={L:5d}: k*/log(sqrtL)={r1:.5f}, "
...
if __name__ == "__main__":
    results, dp = run_analysis(L_max=10000)
