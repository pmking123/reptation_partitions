...
RESULT 2: M4 distinct-part partitions
  The number of partitions of L into distinct parts satisfies:
      log d(L) ~ alpha_d * sqrt(L),   alpha_d = pi/sqrt(3) = alpha_0/sqrt(2)
...
def main():
    ...
    print()
    print("=" * 65)
    print("Rogers-Ramanujan (M5) and distinct-part (M4) entropy coefficients")
    ...
    print(
        f"  alpha_d  = pi/sqrt(3)     = {ALPHA_D:.6f}  (distinct parts, M4 = alpha_0/sqrt(2))"
    )
    print()
    print(
        f"  alpha_RR / alpha_0 = sqrt(2/5) = {RATIO_RR:.6f}"
        f"  ({100 * (1 - RATIO_RR):.1f}% reduction)"
    )
    print(
        f"  alpha_d  / alpha_0 = 1/sqrt(2) = {RATIO_5:.6f}"
    ...
    for L in L_vals:
        ...
        ad = d_map[L]
        print(
            f"  {L:>6}  {ap:>9.6f}  {arr:>10.6f}  "
            f"{ad:>9.6f}  {arr / ap:>8.6f}  {ad / ap:>8.6f}"
        )
    print()
    print(
        f"  Theoretical limits:   {ALPHA_0:.6f}   "
    ...
    with open(combined_csv, "w", newline="") as fh:
        w = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "alpha_RR",
                "alpha_d",
                "alpha_RR_over_alpha_p",
                "alpha_d_over_alpha_p",
            ],
        )
        w.writeheader()
        for L in L_vals:
    ...
    if p[L_ref] > 0 and q[L_ref] > 0 and d[L_ref] > 0:
        ...
        print(f"  At L={L_ref}:")
        print(
            f"    alpha_p  (unrestricted)  = {ap:.5f}  "
            f"(theory {ALPHA_0:.5f}, err {100 * (ap - ALPHA_0) / ALPHA_0:+.2f}%)"
        )
        print(
            f"    alpha_RR (gap-2)         = {arr:.5f}  "
            f"(theory {ALPHA_RR:.5f}, err {100 * (arr - ALPHA_RR) / ALPHA_RR:+.2f}%)"
        )
        print(
            f"    alpha_d  (distinct)      = {ad:.5f}  "
            f"(theory {ALPHA_D:.5f}, err {100 * (ad - ALPHA_D) / ALPHA_D:+.2f}%)"
        )
        print()
        print(
            f"    Observed ratio alpha_RR/alpha_p = {arr / ap:.5f}  "
            f"(theory sqrt(2/5) = {RATIO_RR:.5f})"
        )
        print(
            f"    Observed ratio alpha_d/alpha_p  = {ad / ap:.5f}  "
            f"(theory 1/sqrt(2) = {RATIO_5:.5f})"
        )
        print()
        print(
            f"    Entropy reduction (RR):      {100 * (1 - arr / ap):.1f}%  "
            f"(theory {100 * (1 - RATIO_RR):.1f}%)"
        )
        print(
            f"    Entropy reduction (distinct): {100 * (1 - ad / ap):.1f}%  "
            f"(theory {100 * (1 - RATIO_5):.1f}%)"
        )
    print()
    print("  Note: convergence is O(log(L)/sqrt(L)), not O(1/sqrt(L)).")
    print("  At L=10000 the effective alpha values are still ~5% below theory.")
    print(
        "  Fitted correction coefficient k ~ 0.95 (RR, distinct), ~1.26 (unrestricted)."
    )
