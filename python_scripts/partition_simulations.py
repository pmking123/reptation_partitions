"""
Exact enumeration and verification utilities for the reptation-partition
correspondence.

This module provides:
- Enumerators for compositions/partitions of an integer L.
- Exact sign-assignment counting via partial-sum injectivity:
  count_valid_sign_assignments, w_strong, and w_lambda.
- A brute-force w_lambda checker for cross-validation.

It includes several analysis/verification drivers:
- run_verification_decoupling: single-pass exhaustive check of both
  factorisation (w_strong = w_H * w_V) and injectivity-vs-geometry
  equivalence up to a chosen L_max.
- run_verification_extremal: extremal checks (all-equal subsequences and
  the p=2 closed-form case).
- run_verification_2: analysis of the distinct-parts claim w_H = 2^p and
  systematic counterexample statistics.
- run_verification_3: exact w_lambda-weighted most-probable-partition
  study with tabulated scaling metrics.

When run as a script, it executes these routines and prints summary tables
for selected L_max values.

Usage:
    python partition_simulations.py
...
def run_verification_decoupling(L_max=20):
    """
    Combined exhaustive verification at L <= L_max, in a single pass
    over all signed realisations:
      (a) Factorisation (Theorem decouple): w_strong, computed by joint
          enumeration over all 2^k sign assignments, equals
          w_H(h) * w_V(v) computed per axis.
      (b) Lemma (geometric content): per sign assignment, the partial-sum
          injectivity criterion coincides with the geometric row/column
          condition on the endpoint-augmented segment list.
    """
    t0 = time.time()
    n_comps = n_signed = fact_failures = geo_mismatches = 0
    for L in range(1, L_max + 1):
        for comp in compositions(L):
            ...
            for signs in iproduct((1, -1), repeat=k):
                inj = (partial_sums_distinct(h, signs[0::2]) and
                       partial_sums_distinct(v, signs[1::2]))
                x = y = 0
                rows, cols = [], []
                for i, (r, e) in enumerate(zip(comp, signs)):
                    if i % 2 == 0:
                        rows.append(y); x += e * r
                    else:
                        cols.append(x); y += e * r
                ...
                if k % 2 == 1:
                    ...
                    rows.append(y)
                geo = (len(set(rows)) == len(rows) and
                       len(set(cols)) == len(cols))
                n_signed += 1
                if inj != geo:
                    geo_mismatches += 1
                    print(f"GEO MISMATCH: comp={comp} signs={signs}")
                w_joint += inj
            w_prod = (count_valid_sign_assignments(h) *
                      count_valid_sign_assignments(v))
            n_comps += 1
            if w_joint != w_prod:
                fact_failures += 1
                print(f"FACT FAILURE: comp={comp} "
                      f"joint={w_joint} product={w_prod}")
        print(f"  L={L:2d} done: {n_comps:,} compositions, "
              f"{n_signed:,} signed realisations, "
              f"elapsed={time.time()-t0:.0f}s", flush=True)
    print(f"Factorisation: {n_comps:,} compositions "
          f"(expected {2**L_max - 1:,}), failures: {fact_failures}")
    print(f"Lemma:         {n_signed:,} signed realisations "
          f"(expected {3**L_max - 1:,}), mismatches: {geo_mismatches}")
    return fact_failures == 0 and geo_mismatches == 0

def run_verification_geometric(L_max=16):
    """
    Verify Lemma [geometric content of strong self-avoidance]:
    for every composition of L <= L_max and every sign assignment,
    the partial-sum injectivity criterion (Definition strong_sa)
    coincides with the geometric condition that no two H-type
    segments share a row and no two V-type segments share a column,
    on the segment list augmented by a degenerate transverse segment
    at each chain end.
    """
    ...
    for L in range(1, L_max + 1):
        for comp in compositions(L):
            ...
            for signs in iproduct((1, -1), repeat=k):
                ...
                rows, cols = [], []
                for i, (r, e) in enumerate(zip(comp, signs)):
                    if i % 2 == 0:        # H-run occupies row y
                        rows.append(y)
                        x += e * r
                    else:                 # V-run occupies column x
                        cols.append(x)
                        y += e * r
                cols.append(0)            # degenerate V-segment at start
                if k % 2 == 1:
                    cols.append(x)        # final run H: degenerate V at terminus
                else:
                    rows.append(y)        # final run V: degenerate H at terminus
                geo = (len(set(rows)) == len(rows) and
                       len(set(cols)) == len(cols))
                n_checked += 1
                if inj != geo:
                    mismatches += 1
                    print(f"MISMATCH: comp={comp} signs={signs}")
        print(f"  L={L:2d} done, cumulative checked={n_checked}")
    print(f"Total: {n_checked} signed realisations, mismatches: {mismatches}")
    return mismatches == 0
...
def run_verification_extremal():
    print("=" * 62)
    print("PART 2 Verification 1: Decoupling theorem")
    print("=" * 62)
    print()
    print("Extremal-case checks for Proposition [extremal]: all-equal subsequences and the p=2 formula.")
    print()
